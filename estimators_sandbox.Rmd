...
data_tester <- function(df_exp, df_bench, treat, outcome, 
  ...
  data_for_balance_assess = rbind(df_exp %>% filter(!!sym(treat) == 1) %>% select(treat, outcome, all_of(covars)), df_bench %>% filter(!!sym(treat) == 0) %>% select(treat, outcome, all_of(covars)))
